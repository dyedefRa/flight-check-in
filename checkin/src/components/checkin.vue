          ...
          <span class="rowLetter" v-if="seats % 4 !== 0">{{Seats[seats-1]}}</span>
            ...
            <b-col sm v-else>
              <span v-for="n in 20" :key="n">
                <button v-if="clickedButton == Seats[seats-1]+n" v-bind:class="{button_clicked: isActive}" :disabled="disablebutton" class="button" @click="selectSeat(Seats[seats-1]+n)"></button>
...
export default {
  ...
  methods: {
    ...
    random () {
      ...
      if (ran1 !== 3 && ran1 !== 7) {
        this.$store.state.seat = this.Seats[ran1] + ran2
        this.$store.state.fixedPrice = 0
